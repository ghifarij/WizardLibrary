...
class PotionsViewModel: ObservableObject {
    ...
    func fetchPotions() {
        ...
        let endpoint = PotionsEndpoint.getPotions(page: 1).endpoint
        
        networkService.request(endpoint, responseType: PotterPotionResponse.self)
            ...
            .sink { [weak self] completion in
                ...
                self?.potions = response.data.map { $0.toDomain() }
    ...
    func loadMorePotions() {
        ...
        let endpoint = PotionsEndpoint.getPotions(page: nextPage).endpoint
